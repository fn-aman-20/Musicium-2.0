...
module.exports = {
	...
	category: "🎧 Song",
	usage: "rewind <duration_in_secs>",
	aliases: ["rwd"],
	description: "Rewinds the song by given number of seconds", //the command description for Slash Command Overview
	cooldown: 10,
			...
			try {
				...
				if (!args[0]) {
					return message.reply({
						embeds: [
							...
							.setColor(ee.wrongcolor)
							.setFooter(ee.footertext, ee.footericon)
							.setTitle(`${client.allEmojis.x} **Please mention a Rewind Duration!**`)
							.setDescription(`**Usage**\n\`${client.settings.get(guild.id, "prefix")}rewind <duration_in_secs>\``)
						],
					})
				}
				...
				message.reply({
					embeds: [new MessageEmbed()
					  .setColor(ee.color)
					  .setTimestamp()
					  .setTitle(`⏩ **Rewinded the song ${seekNumber == 0 ? `to start` : `by ${seekNumber} seconds`}!**`)
